...
if __name__ == "__main__":
    ...
    selected = ['difficultProblemCountSolved', 'Level', 'TotalTime', 'cumul_avgstepTimeWE', 'cumul_TotalWETime', 'stepTimeDeviation', 'ruleScoreMP', 'cumul_HoverHintCount']


    print 'Stage 2: Exploitation'
    bestECR = induce_policy_MDP2(original_data, selected)
    random_selected = []
    # for each feature selected, try to remove from the beginning and see if we could find a better one.
    for i in range(8):
        r = random.randint(0, 7)
        while r in random_selected:
            r = random.randint(0, 7)
        random_selected += [r]
        feature = selected[r]
        print '\n************ Attempt to Remove the NO.' + str(r + 1) + ' feature: [' + feature + '] ************'
        tmp_selected = selected[:r] + selected[r+1:]
        rankings = {}
        for f in allFeatures:
        ...
        print 'The highest ECR after removing No.' + str(r+1) + ' feature: ' + str(highECR)
        if highECR > bestECR:
            print '---- Replace [' + feature + '] with [' + highFeature + '] ----'
            print '---- ECR gets improved from ' + str(bestECR) + ' to ' + str(highECR)
            bestECR = highECR
            selected[r] = highFeature
        else:
            print '==== Cannot find a better feature to replace the No.' + str(r+1) + ' feature'
